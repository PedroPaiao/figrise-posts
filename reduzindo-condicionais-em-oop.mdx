...
Muito bem! Agora chegou mais uma tarefa, precisamos especializar o metodo `meals` para retornar o numero de refeições que esse animal precisa dependendo do `kind` dele. Então tivemos a `brilhante` ideia de fazer uma condicional baseando no atributo `kind`
